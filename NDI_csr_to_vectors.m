%% NDI_csv_to_vectors
%Reads one sensor and converts that into a matlab .dat file with vectors in
%it
clear all
close all
clc

%% Read NDI csv file and convert to results file:

%filename = 'NDIdata_snakeravenTestRCM.csv';
filename = 'NDIdata_snakeravenRegistration.csv';
...
r = 11; %start row of transform data (5 by default) %16
...
%% Create data file containing the NDI vectors
...
%Tidy up:
clearvars -except NDI_data 

%Save the Voxel_data
%get rid of of hyphen in the date
save(strcat('NDI_dataRegistration',strrep(date,'-','')),'-v7.3')
